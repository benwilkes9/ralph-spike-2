...
0d. For reference, the application source code is in `src/ralf_spike_2/**/*.py` and tests are in `tests/**/*.py`.
...
ULTIMATE GOAL: A fully-featured Todo CRUD REST API with all CRUD operations, filtering, sorting, and pagination. Consider missing elements and plan accordingly. If an element is missing, search first to confirm it doesn't exist, then if needed author the specification at specs/FILENAME.md. If you create a new element then document the plan to implement it in @IMPLEMENTATION_PLAN.md using a subagent.
